...
const addInstitute = async (req, res) => {
    ...
    const q = "INSERT INTO c_institute (institute_name, institute_location, institute_email) VALUES ( ? )";
...
const getInstitute = async (req, res) => {
    const q = "SELECT * FROM c_institute";
...
const trashInstitute = async (req, res) => {
    ...
    conn.query(q, [id], (err, data) => {
        if (err) {
            console.log(err);
            res.status(500).send({ error: "An error occurred while deleting the institute." });
        } else {
            res.send(data);
...
const editInstitute = async (req, res) => {
    ...
    const q = "UPDATE c_institute SET institute_name = ?, institute_location = ?, institute_email = ? WHERE id = ?";

    conn.query(q, [instituteName, instituteLocation, instituteEmail, id], (err, data) => {
        if (err) {
            console.log(err);
        } else {
            res.send(data);
...
module.exports = { addInstitute, getInstitute, trashInstitute, getPerRecordInstitute, editInstitute }