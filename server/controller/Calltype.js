...
const getCalltype = async (req, res) => {
    const q = "SELECT * FROM c_calltype";
...
const trashCalltype = async (req, res) => {
    ...
    conn.query(q, [id], (err, data) => {
        if (err) {
            console.log(err);
            res.status(500).send({ error: "An error occurred while deleting the Call type." });
        } else {
            res.send(data);
...
module.exports = { addCalltype, getCalltype, trashCalltype, getPerCalltype, editCalltype }