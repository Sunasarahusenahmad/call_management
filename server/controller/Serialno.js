...
const addSerialno = async (req, res) => {
    ...
    const q = "INSERT INTO c_serialno (serial_no, model_id, make_id) VALUES ( ? )";
...
const getSerialno = async (req, res) => {
    const q = "SELECT * FROM c_serialno";
...
const trashSerialno = async (req, res) => {
    ...
    conn.query(q, [id], (err, data) => {
        if (err) {
            console.log(err);
            res.status(500).send({ error: "An error occurred while deleting the serialno." });
        } else {
            res.send(data);
...
const selectedModels = async (req, res) => {
    ...
    conn.query('SELECT * FROM c_model WHERE make_id = ?', [makeId], (error, results) => {
        if (error) {
            console.log(error);
...
const editSerialno = async (req, res) => {
    ...
    const q = "UPDATE c_serialno SET serial_no = ?, model_id = ?, make_id = ? WHERE id = ?";
...
module.exports = { addSerialno, getSerialno, trashSerialno, getPerSerialno, checkSerialno, selectedModels, editSerialno, checkEditSerialno }