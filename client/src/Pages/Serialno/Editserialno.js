...
const Editserialno = () => {
  ...
  const editdata = async (e) => {
    ...
    if (!serialno || !modelId || !makeId) {
      ...
      try {
        ...
        const response = await axios.post("/checkeditserialno", {
          serialno: serialno,
          id: id,
        });
        if (response.data.exists) {
          ...
          await axios
            .put(`/editserialno/${id}`, group)
            .then((res) => {
              console.log(res);
              toast.success("Serial No Updated successfully!");
  ...
  const getPerSno = async () => {
    try {
      const res = await axios.get(`/getperserialno/${id}`);
      setPerSno(res.data);
    } catch (error) {
      console.log(error);
    }
  ...
  const getPerSerialno = async () => {
    try {
      const res = await axios.get(`/getperserialno/${id}`);
      setSerialno(res.data[0].serial_no);
    } catch (error) {
      console.log(error);
    }
  };
