...
const Allmodel = () => {
  ...
  const savedata = async (e) => {
    ...
    if (!modelName || !makeId) {
      ...
      try {
        await axios.post("/addmodel", group)
          .then((res) => {
            console.log(res);
  ...
  const trashModel = async (id) => {
    try {
      if (window.confirm("Are you sure you want to remove this Model")) {
        const res = await axios.delete(`/trashmodel/${id}`);
        window.location.reload();
        getModel();
      }
    } catch (error) {
                            ...
                            <table class="table" id="dataTable" width="100%">
                              <thead>
                                <tr style={{ cursor: 'pointer' }}>
                                  <th scope="col">ID</th>
                                  <th onClick={toggleSortDirection}>Model Name <span>
                                        ...
                                        <td>
                                          ...
                                          <p className="p-0 m-0 tred " style={{ fontSize: '14px' }}>
                                            <div className="d-flex">
                                              ...
                                              <Link to={'/editmodel'} state={{ id: item.id }} className='text-decoration-none'><p className="p-0 m-0">Edit | </p></Link>
                                              <p className="text-danger p-0 m-0" onClick={() => { trashModel(item.id) }}>&nbsp;Trash </p>
                                            </div>
                                          </p>
                                        </td>
                                        <td>
                        ...
                        <div className="card-body" style={{ fontFamily: 'Signika Negative', fontSize: '15px' }}>
                          <form onSubmit={savedata}>
                            ...
                            <div class="form-group">
                              ...
                              <input type="text" class="form-control" placeholder="Enter Model Name" value={modelName} onChange={(e) => { setModelName(e.target.value) }} />
                            </div>
